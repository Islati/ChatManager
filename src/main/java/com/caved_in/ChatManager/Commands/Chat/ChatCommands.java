		...
		if (!commandArgs[0].isEmpty()) {
			...
			if (!commandArgs[1].isEmpty()) {
				...
				if (PlayerHandler.isOnline(messagingPlayer)) {
					PlayerHandler.getPlayer(messagingPlayer).sendMessage(ChatColor.WHITE + "[" + ChatColor.YELLOW + player.getDisplayName() + ChatColor.AQUA +
							" -> " + ChatColor.GREEN + "You" + ChatColor.WHITE + "] " + Message);
				}
				PlayerHandler.getPlayer(messagingPlayer).sendMessage(ChatColor.WHITE + "[" + ChatColor.YELLOW + "You" + ChatColor.AQUA + " -> " + ChatColor
						.GREEN + player.getDisplayName() + ChatColor.WHITE + "] " + Message);
			}
		}
	}
