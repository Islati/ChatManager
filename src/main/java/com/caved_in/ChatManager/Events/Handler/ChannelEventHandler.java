		...
		if (!event.isCancelled())
		{
			...
			if (!channelName.equalsIgnoreCase(ChatManager.GLOBAL_CHAT_CHANNEL))
			{
				if (PlayerHandler.isOnline(playerName))
				{
					...
					{
						globalChatChannel.sendToMembers(StringUtil.formatColorCodes(String.format("&7%s has joined the Channel!", playerName)));
					}
				}
			}
			...
			if (chatChannel.isPrivate() && !chatChannelCreator.equalsIgnoreCase(playerName))
			{
				if (ChatManager.channelHandler.hasChannelInvitation(playerName))
				{
					if (ChatManager.channelHandler.acceptChannelInvitation(playerName))
					{
						...
						{
							chatChannel.sendToMembers(StringUtil.formatColorCodes(String.format("&7%s has joined the Channel!", playerDisplayName)));
						}
					}
					else
					{
						player.sendMessage(StringUtil.formatColorCodes("&cFailed to join the channel."));
					}
				}
				else
				{
