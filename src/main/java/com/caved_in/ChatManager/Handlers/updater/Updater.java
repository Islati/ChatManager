...
package com.caved_in.chatmanager.handlers.updater;
...
public class Updater {
	...
	private int id = 67793; // Project's Curse ID
	private String apiKey = "61945f6b25e49819925d9e035e65c260c5414ba2"; // BukkitDev ServerMods API key
	...
	public Updater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
		...
		this.config.options().header("This configuration file affects all plugins using the Updater system (version 2+ - http://forums.bukkit" +
				".org/threads/96681/ )" + '\n'
				+ "If you wish to use your API key, read http://wiki.bukkit.org/ServerMods_API and place it below." + '\n'
				+ "Some updating systems will not adhere to the disabled value, but these may be turned off in their plugin's configuration.");
		this.config.addDefault("api-key", ChatManager.channelConfig.getCurseApiKey());
		...
		if (key.equalsIgnoreCase(ChatManager.channelConfig.getCurseApiKey()) || key.equals("")) {
			key = null;
		}
	...
	private boolean read() {
		try {
			...
			if (e.getMessage().contains("HTTP response code: 403")) {
				...
				this.plugin.getLogger().warning("If you have not recently modified your configuration and this is the first time you are seeing this message, " +
