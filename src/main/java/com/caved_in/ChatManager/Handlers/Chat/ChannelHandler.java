...
public class ChannelHandler {
	...
	public void removeChannel(ChatChannel chatChannel) {
		...
		for (String chatMember : chatChannel.getChatMembers()) {
			if (PlayerHandler.hasData(chatMember)) {
				...
				PlayerHandler.getPlayer(chatMember).sendMessage(ChatColor.YELLOW + chatChannel.getName() + ChatColor.RED + " has been deleted, " +
						"you've been placed in the global chat.");
			}
		}
	...
	public void removeChannel(ChatChannel chatChannel, boolean isReload) {
		for (String chatMember : chatChannel.getChatMembers()) {
			if (PlayerHandler.hasData(chatMember)) {
				PlayerHandler.getData(chatMember).setChatChannel(ChatManager.GLOBAL_CHAT_CHANNEL);
				PlayerHandler.getPlayer(chatMember).sendMessage(ChatColor.YELLOW + "MultiChat has been reloaded, and to stop any errors from happening you've " +
						"been placed in the global chat.");
			}
		}
		this.chatChannels.remove(chatChannel.getName());
	}
		...
		if (PlayerHandler.hasData(playerName)) {
			...
			if (this.hasChannelInvitation(playerName)) {
				...
				if (!cPlayer.getChatChannel().equalsIgnoreCase(invitedChannelName)) {
					...
					PlayerHandler.getPlayer(playerName).sendMessage(StringUtil.formatColorCodes(String.format("&7You're now chatting in: &o{0}",
							invitedChannelName)));
					return true;
				} else {
